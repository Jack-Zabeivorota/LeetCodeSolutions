...
class Solution1001:
    def __is_illuminated(self, lamp: Tuple[int, int], n: int) -> int:
        y, x = lamp
        
        if y in self.horizontals or x in self.verticals:
            return 1

        if x - y in self.left_diagonal or n - 1 - x - y in self.right_diagonal:
            return 1
    ...
    def gridIllumination(self, n: int, lamps: List[List[int]], queries: List[List[int]]) -> List[int]:
        '''
        Є сітка розміром `n * n` клітинок, на ній розташовані лампи, позиції яких указані в `lamps` (`[row,col]`).
        Лампи освітлюють свою вертикаль, горизонталь та діагоналі відносно позиції лампи.
        В `queries` указані 
        ...
        for lamp in queries:
            ans.append(self.__is_illuminated(lamp, n))
            self.__remove_ligths_from_range(lights, lamp, n)
